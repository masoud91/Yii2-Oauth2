 ...
 * @package common\idco\OAuth2\GrantType
 */
class AuthorizationCode implements GrantTypeInterface
{
    ...
    {
        if (!$request->request('code') || !$request->request("uuid")) {
            $response->setError(401, 'invalid_request', 'Missing parameter: "code" and "uuid" are required');
        ...
        if (!$authCode = $this->storage->getAuthorizationCode($code)) {
            $response->setError(401, 'invalid_grant', "Authorization code doesn't exist or is invalid for the client");
        ...
        if (isset($authCode['redirect_uri']) && $authCode['redirect_uri']) {
            if (!$request->request('redirect_uri') || urldecode($request->request('redirect_uri')) != urldecode($authCode['redirect_uri'])) {
                $response->setError(401, 'redirect_uri_mismatch', "The redirect URI is missing or do not match", "#section-4.1.3");
        ...
        if (!isset($authCode['expires'])) {
            throw new \Exception('Storage must return auth code with a value for "expires"');
        }

        if ($authCode["expires"] < time()) {
            $response->setError(401, 'invalid_grant', "The authorization code has expired");
